...
// The same positions through PlayerScoreWith, where TTBITS=0 really means
// no table (the move-picking root always builds one). For measuring what
// the table costs per node.
func TestProfileScoreWorkload(t *testing.T) {
	if os.Getenv("PROFILE") == "" {
		t.Skip("set PROFILE=1")
	}
	net, err := LoadHalfKPNet("../champion_net.json")
