...
func TestProbeWithMoveExact(t *testing.T) {
	...
	tt.storeWithMove(key, 1.5, 4, ttExact, board.White, m)

	score, cutoff, move, okMove := tt.probeWithMove(key, 4, board.White, -2.0, 2.0)
	if !cutoff || score != 1.5 {
...
func TestProbeWithMoveNoCutoffReturnsMove(t *testing.T) {
	...
	tt.storeWithMove(key, 1.5, 2, ttExact, board.White, m)

	_, cutoff, move, okMove := tt.probeWithMove(key, 4, board.White, -2.0, 2.0)
	if cutoff {
...
func TestProbeWithMoveMiss(t *testing.T) {
	...
	_, cutoff, _, okMove := tt.probeWithMove(key, 4, board.White, -2.0, 2.0)
	if cutoff || okMove {
