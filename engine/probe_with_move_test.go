...
func TestProbeWithMoveExact(t *testing.T) {
	...
	score, cutoff, move, okMove := tt.probeWithMove(key, 4, 0, 0, board.White, -2.0, 2.0)
	if !cutoff || score != 1.5 {
...
func TestProbeWithMoveNoCutoffReturnsMove(t *testing.T) {
	...
	_, cutoff, move, okMove := tt.probeWithMove(key, 4, 0, 0, board.White, -2.0, 2.0)
	if cutoff {
...
func TestProbeWithMoveMiss(t *testing.T) {
	...
	_, cutoff, _, okMove := tt.probeWithMove(key, 4, 0, 0, board.White, -2.0, 2.0)
	if cutoff || okMove {
